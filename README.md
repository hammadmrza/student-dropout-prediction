...
**Top predictors (SHAP):** 2nd semester courses approved (0.110) · 1st semester approved (0.050) · 2nd semester grade (0.046) · Tuition fees up to date (0.030)
...
### Prerequisites
```bash
pip install pandas numpy scikit-learn xgboost imbalanced-learn shap matplotlib seaborn streamlit plotly optuna
```
...
| 10 | Hyperparameter tuning with Optuna (RF, XGBoost, LR, tuned stacking) |
| 11 | Summary |
...
Optuna TPE sampler with 100 trials (RF, XGBoost) and 50 trials (LR). Tuning performed on **original unbalanced data** to find parameters that generalize to real class distributions, then final models trained on SMOTE-balanced data.
